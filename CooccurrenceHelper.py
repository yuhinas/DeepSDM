...
class CooccurrenceHelper():
    def __init__ (self,
        ...
        # whether to embedding with virtual species
...
class VirtualCooccurrenceHelper():
    def __init__ (self, 
                  spatial_conf, 
                  temporal_conf, 
                  virtual_conf, 
                  input_env, 
                  output_dir = './workspace/species_data_virtual'):
        
        super().__init__()
        
        # create a full spatial_conf
        self.spatial_conf = spatial_conf
        ...
        self.x_start = self.spatial_conf.x_start
        self.y_start = self.spatial_conf.y_start
        ...
        # set output directory and create folders
        output_dir = f'{output_dir}_{virtual_conf.version}'
        self.output_dir = output_dir
    ...
    def aggregate_cooccurrence_units_virtual(self, sp_filter_from = None, 
                                     cooccurrence_day_mul=1, cooccurrence_xy_mul=1):
        start_time = time.time()
        def res_divider(a, b):
            digit_parts = str(b).split('.')
        ...
        if sp_filter_from is None:
            sp_filter_from = [f'./virtual/{self.virtual_conf.version}/species_occurrence_virtual.csv', f'./virtual/{self.virtual_conf.version}/species_occurrence_realworld.csv']
        sp_filter_list = []
        for sp_filter_element in sp_filter_from:
            sp_filter_list.append(pd.read_csv(sp_filter_element))
        self.species_filter = pd.concat(sp_filter_list)
        self.species_filter.to_csv(f'{self.occurrence_dir}/species_occurrence_realworldvirtual_combine.csv')
    ...
    def count_cooccurrence_mod_virtual(self, cooccurrence_counts_file='cooccurrence_virtualrealworld.csv'):
        start_time = time.time()
        data_unit = self.data_unit
        primary_indices = {}
        cooccur_counts_df = None
