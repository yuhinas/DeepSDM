...
class TaxaDataset_smoothviz(Dataset):
    def __init__(self, idx_species_date, env_stack, embedding, label_stack, subsample_height, subsample_width, num_smoothviz_steps, device):
        ...
        self.embedding = torch.tensor(embedding[self.species]).reshape(-1, 1, 1).to(self.device)#.cuda()
        ...
        for i in range(len(env)):
            env_new.append(env[i:(i+1), :, :].to(self.device))
...
#                                  mode = 'replicate').to(torch.float))
        ...
        self.env = F.pad(torch.cat(env_new).to(self.device),
                         (self.subsample_width, 2 * self.subsample_width, self.subsample_height, 2 * self.subsample_height), 
                         mode = 'replicate').to(self.device) #.to(torch.float)
    ...
    def async_cuda(self):
        self.env = self.env.cuda(device = self.device, non_blocking=True)
        self.embedding = self.embedding.cuda(device = self.device, non_blocking=True)
#         return self
